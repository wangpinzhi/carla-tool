...
if __name__ == '__main__':
    ...
    # basic settings
    parser.add_argument('--fov', type=int, default=110, help='target fisheye fov')
    ...
    phi = np.arctan2(y, np.sqrt(x * x + z * z)).astype(np.float32)
    theta = theta / np.pi
    phi = phi / np.pi * 2.0
    ...
    for frame in tqdm(frames, desc=f'c2p {args.camera}', unit='frames'):
        ...
        torch_erp = cubemap2erp.ToEquirecTensor(cube_tensor)

        out_pinhole = F.grid_sample(torch_erp, grid, mode='bilinear', padding_mode='zeros', align_corners=True)
