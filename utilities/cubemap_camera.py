...
def cubemap_data_callback(sensor_data, sensor_queue:Queue, sensor_name:str, view:str):
...
'''
def get_cubemap_camera_rgb(world, target_vehicle, sensor_queue, settings:list):
    ...
    for idx, cur_setting in  enumerate(settings):
        ...
        for view, rotation_pitch, rotation_yaw, rotation_roll in items:
            ...
            carla.Transform(carla.Location(x=cur_setting['location_x'],y=cur_setting['location_y'],z=cur_setting['location_z']),
                            carla.Rotation(pitch=rotation_pitch,yaw=rotation_yaw+cur_setting['rotation_yaw'],roll=rotation_roll)
            ), attach_to=target_vehicle)

            actor.listen(lambda data, s_name=cur_setting["name"], s_view=view: cubemap_data_callback(data, sensor_queue, s_name, s_view))
            actors.append(actor)
...
'''
def get_cubemap_camera_depth(world, target_vehicle, sensor_queue, settings:list):
