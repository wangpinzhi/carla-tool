...
def get_cubemap_camera_rgb(world, target_vehicle, sensor_queue, settings: list):
    ...
    for idx, cur_setting in enumerate(settings):
        ...
        items = [('front', 0, 0, 0), ('right', 0, 90, 0), ('back', 0, 180, 0), ('left', 0, -90, 0), ('up', 90, 0, 0),
                 ('down', -90, 0, 0)]
        for view, rotation_pitch, rotation_yaw, rotation_roll in items:
            ...
            actor.listen(
                lambda data, s_name=cur_setting["name"], s_view=view: cubemap_data_callback(data, sensor_queue, s_name,
                                                                                            s_view))
            actors.append(actor)
